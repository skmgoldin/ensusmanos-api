...
class AuthTest {
  ...
  public AuthTest() throws Exception {
    ...
    AttachedResources attachedResources =
        new AttachedResources(
            new String[] {"SYSTEM_KEY", "USERS_DB"}, "jonet", environment.getEnvar("JONET_ENV"));
    auth = new Auth(attachedResources);

    SYSTEM_KEY = auth.fetchSystemKey();

    AttributeValue testUserEmail = AttributeValue.builder().s("joann.arosemena@gmail.com").build();
